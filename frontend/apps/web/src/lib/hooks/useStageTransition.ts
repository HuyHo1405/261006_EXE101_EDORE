 ...
 *
 * Returns:
 *   visibleStage   — the stage that is currently rendered (lags behind during exit)
 *   isExiting      — true during the exit animation; consumer applies `.stage-exit`
 *   goTo(newStage) — trigger a transition to a new stage
 *
 * Timing:
 *   EXIT_MS  (160ms) — matches .stage-exit animation duration in globals.css
...
export function useStageTransition(initialStage = 'input') {
  const [visibleStage, setVisibleStage] = useState(initialStage)
  const [isExiting, setIsExiting] = useState(false)
  const timerRef = useRef<ReturnType<typeof setTimeout> | null>(null)

  const goTo = useCallback((newStage: string) => {
