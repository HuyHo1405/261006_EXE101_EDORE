...
def retrieve_top_k(
    ...
    if not query_tokens:
        # Nếu không có query hints → fallback lấy k chunks đầu tiên chưa dùng
        fallback_indices = [i for i in range(len(chunks)) if i not in exclude_indices][:k]
        if len(fallback_indices) < k:
            fallback_needed = k - len(fallback_indices)
            fallback_indices.extend([i for i in range(len(chunks)) if i in exclude_indices][:fallback_needed])
        fallback_indices.sort()
        selected = [chunks[i] for i in fallback_indices]
        if return_indices:
            return selected, fallback_indices
        return selected
    ...
    # Lọc ra các indices chưa dùng và score > 0
    available_scored = [(i, score) for i, score in scored if score > 0 and i not in exclude_indices]

    # Nếu không đủ k chunks, bổ sung thêm từ các chunks đã loại trừ nhưng score > 0
    if len(available_scored) < k:
        already_used_scored = [(i, score) for i, score in scored if score > 0 and i in exclude_indices]
        needed = k - len(available_scored)
        available_scored.extend(already_used_scored[:needed])

    top_k_indices = [i for i, score in available_scored[:k]]

    # Nếu không có chunk nào khớp → fallback lấy k chunk chưa dùng
    if not top_k_indices:
        fallback_indices = [i for i in range(len(chunks)) if i not in exclude_indices][:k]
        if len(fallback_indices) < k:
            fallback_needed = k - len(fallback_indices)
            fallback_indices.extend([i for i in range(len(chunks)) if i in exclude_indices][:fallback_needed])
        top_k_indices = fallback_indices
...
def get_all_section_contexts(
    ...
    """
    Tạo context riêng biệt cho từng node trong template với cơ chế deduplicate.
    ...
    # event_boundary_chunk được gọi ngay trong semantic_chunk
    chunks = semantic_chunk(text)
    used_indices = set()
